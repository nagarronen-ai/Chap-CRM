...
export default function Contacts() {
  ...
  useEffect(() => {
    ...
    setFiltered(result);
  }, [search, filterStage, filterOrigin, filterCategory, companies]);
    ...
    <div style={{ display: 'flex', fontFamily: 'Inter, sans-serif', background: '#F5F3EF', minHeight: '100vh' }}>
      ...
      <div style={{ marginLeft: 240, flex: 1, padding: 40 }}>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 24, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          {(search || filterStage || filterOrigin || filterCategory) && (
            <button onClick={() => { setSearch(''); setFilterStage(''); setFilterOrigin(''); setFilterCategory(''); }}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#F5F3EF', borderBottom: '1px solid rgba(62,66,61,0.1)' }}>
                {['Company', 'Category', 'Location', 'People', 'Origin', 'Stage', ''].map(h => (
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={7} style={{ padding: 40, textAlign: 'center', color: '#717182' }}>Loading...</td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={7} style={{ padding: 40, textAlign: 'center', color: '#CBCED4' }}>No companies found</td></tr>
              ) : filtered.map((c, i) => (
                <tr key={c.id}
          ...
          if (pendingForm?.first_name) {
            ...
            try {
              await axios.post(`${API}/contacts/companies/${dupCompany.id}/people`, {
                first_name: pendingForm.first_name,
                last_name: pendingForm.last_name,
                title: pendingForm.title,
                email: pendingForm.email,
                work_phone: pendingForm.work_phone,
              }, { headers });
            } catch (err) { console.error(err); }
            setSaving(false);
          }
